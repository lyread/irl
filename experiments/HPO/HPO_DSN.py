...
class MyCallback(Callback):
    ...
    def on_tell_end(self, smbo, info, value) -> bool | None:
        ...
        if self.trials_counter % 10 == 0:
            ...
            print(f"Time consumption:{self.time - time.time()} seconds")
...
def collect_experience(env, algorithm: Sarsa):
    """
...
if __name__ == "__main__":
    ...
    cs.add_hyperparameter(
        CategoricalHyperparameter(
            "discount_factor", np.arange(0.99, 0.71, -0.01).tolist(), default_value=0.99
        )
    )
    cs.add_hyperparameter(
        CategoricalHyperparameter(
            "epsilon", np.arange(0.1, 0.61, 0.01).tolist(), default_value=0.1
        )
    )
    cs.add_hyperparameter(
        CategoricalHyperparameter(
    ...
    cs.add_hyperparameter(
        CategoricalHyperparameter(
            "batch_size", [i for i in range(16, 129, 8)], default_value=64
        )
    )
    cs.add_hyperparameter(
        CategoricalHyperparameter(
    ...
    scenario = Scenario(cs, deterministic=True, n_trials=NUM_TRIALS, n_workers=4)
    ...
    save_result_as_pickle(result, folder_name, "smac_results_dsn_pT")
